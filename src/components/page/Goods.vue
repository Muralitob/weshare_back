  ...
  <div class="table">
    ...
    <div class="container">
      ...
      >
        ...
        <el-table-column prop="title" label="物品" width="150"></el-table-column>
        <el-table-column prop="account" label="提交用户账户" width="120">
          <template slot-scope="scope">
            <span>{{scope.row.user.account}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="degree" label="新旧程度" width="120">
        </el-table-column>
        <el-table-column prop="type" label="类型" width="120">
          <template slot-scope="scope">
            <span>{{usedTypeMap[scope.row.type]}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="状态" width="120">
          <template slot-scope="scope">
            <span v-if="scope.row == 0">未完成</span>
            <span v-else>已完成</span>
          </template>
        </el-table-column>
        <el-table-column prop="release_time" label="发布时间">
            <template slot-scope="scope">
                {{convertUTCTimeToLocalTime(scope.row.release_time)}}
            </template>
        </el-table-column>
